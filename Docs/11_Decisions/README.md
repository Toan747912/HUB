...
| [DECISION-042](DECISION-042-Database-Naming-Convention-Alignment.md) | Database Naming Convention Alignment — `snake_case` for PostgreSQL/Supabase (supersedes PascalCase) | Accepted (Locked) — closes SupabaseCompatibilityReview.md High-impact item #2 |
| [DECISION-043](DECISION-043-Supabase-Auth-Alignment.md) | Supabase Auth Alignment — `learner.id = auth.users.id` (shared UUID, no separate mapping) | Accepted (Locked) — closes SupabaseCompatibilityReview.md High-impact item #1 |
| [DECISION-044](DECISION-044-Versioning-Strategy.md) | Versioning Strategy — `version_number` (trigger-incremented bigint), not `rowversion`-equivalent or `updated_at`-only | Accepted (Locked) — closes SupabaseCompatibilityReview.md Medium-impact Versioning item |
| [DECISION-045](DECISION-045-Temporal-Strategy.md) | Temporal Strategy — trigger-maintained History Tables, only where no companion log exists | Accepted (Locked) — closes SupabaseCompatibilityReview.md Medium-impact Temporal item |
