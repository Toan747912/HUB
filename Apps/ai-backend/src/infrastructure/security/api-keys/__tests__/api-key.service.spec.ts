...
describe('ApiKeyService', () => {
  ...
  beforeEach(() => {
    ...
    service = new ApiKeyService(repository as unknown as ApiKeyRepository, auditLog as unknown as AuditLogService);
  });

  // Evidence: API key
  it('issues a raw key and stores only its SHA-256 hash', async () => {
    repository.create.mockResolvedValue({ _id: 'key-1', keyHash: 'x', label: 'ci', createdAt: new Date(), revokedAt: null, permissions: [] });
    ...
    expect(auditLog.recordSecurityEvent).toHaveBeenCalledWith(expect.objectContaining({ operation: 'API_KEY_ISSUED' }));
  });
  ...
  it('issues a key scoped to explicitly requested permissions', async () => {
    ...
    expect(repository.create).toHaveBeenCalledWith(expect.any(String), 'integration', ['Goal.Read']);
  });

  it('verify() returns the matching document for a key with an active matching hash', async () => {
    const doc = { _id: 'key-1', keyHash: 'x', label: 'ci', createdAt: new Date(), revokedAt: null, permissions: [] };
  ...
  it('revoke() audits an API_KEY_REVOKED event', async () => {
    ...
    expect(auditLog.recordSecurityEvent).toHaveBeenCalledWith(expect.objectContaining({ operation: 'API_KEY_REVOKED' }));
  });
});
