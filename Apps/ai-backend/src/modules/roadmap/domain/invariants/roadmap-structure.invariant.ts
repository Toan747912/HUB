...
export const ensureNonEmptyPlan = (phases: RoadmapPhase[]): void => {
  ...
  if (!hasMilestones) {
    throw new RoadmapDomainError('ROADMAP_PLAN_EMPTY', 'Every phase must contain at least one milestone');
  }

  const hasTasks = phases.every((phase) => phase.milestones.every((milestone) => milestone.tasks.length > 0));
  if (!hasTasks) {
    throw new RoadmapDomainError('ROADMAP_PLAN_EMPTY', 'Every milestone must contain at least one ordered task');
  }
};
