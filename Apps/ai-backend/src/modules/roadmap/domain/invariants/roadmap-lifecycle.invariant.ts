...
export const ensureValidLifecycleTransition = (from: RoadmapStatusValue, to: RoadmapStatusValue): void => {
  if (!ALLOWED_TRANSITIONS[from]?.includes(to)) {
    throw new RoadmapDomainError('INVALID_STATE_TRANSITION', `Invalid transition from ${from} to ${to}`);
  }
};
