...
export class HttpExceptionFilter implements ExceptionFilter {
    ...
    const req = ctx.getRequest<RoadmapRequestWithTrace & Request>();
  ...
  private normalize(exception: unknown): { status: number; error: ErrorEnvelope } {
    if (exception instanceof RoadmapNotFoundError) {
      return {
    ...
    if (exception instanceof RoadmapVersionConflictError) {
      return {
    ...
    if (exception instanceof RoadmapValidationError || exception instanceof RoadmapStateTransitionError) {
      return {
