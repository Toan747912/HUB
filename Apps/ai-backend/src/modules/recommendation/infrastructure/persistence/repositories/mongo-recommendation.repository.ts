...
export class MongoRecommendationRepository implements IRecommendationRepository {
  ...
  async findById(id: string): Promise<Recommendation | null> {
    return this.instrumented('findById', id, async () => {
      const start = Date.now();
      try {
        const doc = await this.model.findById(id).lean<RecommendationDocument>().exec();
        this.log('findById', id, start, 'SUCCESS');
        return doc ? RecommendationPersistenceMapper.toDomain(doc) : null;
      } catch (error) {
  ...
  async findAll(learnerId?: string): Promise<Recommendation[]> {
    return this.instrumented('findAll', learnerId ?? '*', async () => {
      ...
      try {
        ...
        const docs = await this.model.find(filter).lean<RecommendationDocument[]>().exec();
