...
export class HttpExceptionFilter implements ExceptionFilter {
    ...
    const req = ctx.getRequest<AssessmentRequestWithTrace & Request>();
  ...
  private normalize(exception: unknown): { status: number; error: ErrorEnvelope } {
    if (exception instanceof AssessmentNotFoundError) {
      return {
    ...
    if (exception instanceof AssessmentVersionConflictError) {
      return {
    ...
    if (exception instanceof AssessmentValidationError || exception instanceof AssessmentStateTransitionError) {
      return {
